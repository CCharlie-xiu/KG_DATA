...
export default function Masonry({
  ...
  const { grid, totalHeight } = useMemo(() => {
    if (!width) {
      return {
        grid: [] as Array<MasonryItem & { x: number; y: number; w: number; h: number }>,
        totalHeight: 0,
      };
    }
    ...
    return { grid: next, totalHeight: Math.max(0, ...colHeights) };
  }, [columns, items, width]);
  ...
  const handleMouseLeave = (e: MouseEvent<HTMLDivElement>, item: MasonryItem) => {
    const element = e.currentTarget;
    const selector = `[data-key="${item.id}"]`;
