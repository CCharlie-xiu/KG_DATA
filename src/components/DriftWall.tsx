    ...
    const animate = (ts: number) => {
      ...
      if (!reduced) {
        for (let c = 0; c < trackRefs.current.length; c++) {
          ...
          if (!meta) continue;
          const paused = (wallHoveredRef.current && pauseOnHover) || hoveredColRef.current === c;
          const target = baseVelocities[c] * (paused ? 0 : 1);
  ...
  const renderTile = (item: DriftWallItem, id: string, colIndex: number) => {
    const inner = (
      <div className="drift-wall__inner">
        <div className="drift-wall__copy">
          {item.kicker ? <span className="drift-wall__kicker">{item.kicker}</span> : null}
          <strong className="drift-wall__title">{item.title}</strong>
          {item.excerpt ? <p className="drift-wall__excerpt">{item.excerpt}</p> : null}
        </div>
    ...
    if (item.href) {
      return (
        <Link key={id} to={item.href} {...common}>
          {inner}
        </Link>
      );
    }
